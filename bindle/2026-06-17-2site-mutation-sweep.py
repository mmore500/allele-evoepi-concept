...
def delimit_data(mo):
    ...
    notebook `bindle/2026-05-20-founder.py`), cached as a parquet on OSF.
    The sweep fixes `N_SITES=2` (genomes `00, 01, 10, 11`, Hamming
    weights `0 .. 2`) and is *configured* to step `MUTATION_RATE` across
    17 conditions spanning `1e-9` to `1e-1` (~2 points per decade) at 200
    replicates each, 5000 steps per replicate, POP_SIZE=1_000_000, on CPU
    (engine=numpy). The current `hw-final` export covers the **7 fastest
    conditions** (`1e-4 .. 1e-1`); the notebook plots whatever rates are
    present, so it will fill in the slower rates once they are exported.

    The dataframe has one row per `(replicate_uid, hw)` at the final
    simulation step recording the per-Hamming-weight number of cases
    (`n_cases`), alongside the simulation parameters --- notably the
    swept `mutation_rate` --- as constant-valued columns. The OSF slug is
    downloaded with `requests` and cached at `/tmp/<slug>` so re-runs hit
    the local copy.
    """
    )
    return
...
def configure_args(mo):
    # CLI args. Defaults pull the 2-site mutation-sweep hw-final parquet
    # that backs this notebook (OSF https://osf.io/x7qmt).
    _args = mo.cli_args()
    OSF_SLUG = str(_args.get("osf-slug") or "x7qmt")
    OSF_URL = str(
