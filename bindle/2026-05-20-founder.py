...
def delimit_simulation(mo):
    ...
    * `--mutation-rate` --- per-genome mutation rate (default `1e-5`).
...
def run_founder(
    ...
    # the exception --- it is supplied via the `--mutation-rate` CLI arg
    # (default 1e-5) so it can be swept independently.
    CONTACT_RATE = 0.35
    RECOVERY_RATE = 0.1
    WANING_RATE = 0.01
    IMMUNE_STRENGTH = 0.7
    ...
    print(
        ...
        f"mutation_rate={MUTATION_RATE} uid={replicate_uid} ===",
    )
