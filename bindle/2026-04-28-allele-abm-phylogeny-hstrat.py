...
def delimit_simulation(mo):
    ...
    This notebook is a variant of `2026-04-28-allele-abm-phylogeny.py` that
    swaps exact phylogeny tracking for *hereditary stratigraphic surface*
    annotations (see https://hstrat.rtfd.io). Each infected host carries a
    `dstream_S=64`-site, 64-bit "hybrid" surface that ingests one differentia
    per simulation step; on transmission the donor surface state is copied to
    the recipient (the ABM analogue of `CloneDescendant`). At up to
    `MAX_SAMPLED_TAXA` infected hosts are snapshotted each step, so a single
...
def run_phylogeny_sweep(
    ...
    # Sweep at the largest genome (`N_SITES=16`) only, but for longer
    # (8,000 steps × 3 replicates) so the resulting reconstructed
    # phylogenies have time to coalesce into informative topology.
    # `MAX_SAMPLED_TAXA` caps memory; it bounds the number of
    # (snapshot × infected-host) sample rows that flow into
    # `surface_unpack_reconstruct`. With `MAX_SAMPLED_TAXA=1` and
    # `SNAPSHOT_INTERVAL=1`, an 8,000-step run produces ~8k records
    # (each ~1 KB of hex), keeping the records dataframe +
    # reconstruction intermediate buffers well under the ~12 GB
    # GitHub-runner memory budget. Use the canonical 64-bit hybrid
