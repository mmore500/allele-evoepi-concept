...
def delimit_data(mo):
    ...
    `/tmp/<slug>` so re-runs hit the local copy. It renders the
    establishment-phase trellis at **three zoom windows** --- the first
    `2000`, `400`, and `200` updates --- each excluding the slowest
    `1e-9` mutation rate.
    """
    )
    return
...
def delimit_prep(mo):
    ...
    that, for a given **step window**, clips both frames to the first
    `step_clip` updates and renders the replicate trellis: **one column
    per `mutation_rate`** (power-of-ten rates only, the slowest `1e-9`
    dropped, increasing **left to right**) and **one row per within-rate
    replicate** (ordered by `replicate_uid`). Per-step series are
    subsampled by a window-dependent `STRIDE` so each line carries at
    most ~1000 points.
    """
    )
    return
...
def def_make_trellis(mlines, pathlib, plt, sns, tp):
    ...
    def make_trellis(hw_df, strain_hw_df, N_SITES, step_clip):
        ...
        # Power-of-ten mutation rates, slowest 1e-9 dropped, ascending.
        rate_vals = [
            _r
        ...
        n_rows = int(max_reps)
        print(
            f"step_clip={step_clip} stride={stride} "
        ...
        with tp.teed(
            ...
            fig.suptitle(f"first {step_clip} updates", y=1.02, fontsize=11)
            fig.legend(
                handles=_handles,
                loc="upper center",
...
def delimit_plot_2000(mo):
    ...
    trajectories, color-coded by Hamming weight; dashed lines are the
    aggregated **Hamming-weight band** prevalences in the same scheme.
    These are individual replicate trajectories, so **no confidence
    interval** is drawn. The widest zoom window resolves the full
    establishment phase and the start of the endemic plateau.
    """
    )
    return
...
def delimit_plot_400(mo):
    ...
    The same trellis clipped tighter, to the first **400** updates, to
    resolve the early outbreak peak and the first strain-divergence
    transient before the bands settle.
    """
    )
    return
