...
# Sweep: the 4-site model across the high-mutation-rate half of the
# geometric rung sequence used by the other mutation sweeps, ~2 points
# per decade from 1e-1 down to (but not including) 1e-6, i.e. down to
# 3e-6. This is the complement of the 2026-08-26 4-site sweep, which
# covers 1e-5 down to 3e-11 --- together the two sweeps span the full
# rung sequence from 1e-1 to 3e-11 with no gap or overlap at 1e-6/1e-5.
# 200 replicates per rate.
...
# the original 2-site/3-site sweeps), not the deep 36h/150,000-step
# budget used by the 2026-08-26 4-site sweep: N_STEPS=5000 is plenty
# at these higher mutation rates, where dynamics saturate quickly.
#
