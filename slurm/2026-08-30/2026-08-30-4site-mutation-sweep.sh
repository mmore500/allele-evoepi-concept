...
# Sweep: the 4-site model across the ten rungs immediately below the
# lower boundary (3e-6) of the 2026-08-29 wide sweep, i.e. 1e-6 down to
# 3e-11 --- the same number (10) and per-condition replicate count
# (200) of 4-hour runs as that script, just shifted one decade lower.
# This range coincides with the bottom 10 of the 12 rungs already
# covered by the deep 2026-08-26 sweep (1e-5 down to 3e-11, 25
# replicates, 150,000 steps, 36h); this script adds a much larger
# (200-replicate) but shallower (5000-step, 4h) batch at those same
# low rates, the same way 2026-08-29 did for the high-rate half.
# 10 MUTATION_RATE conditions x 200 replicates = 2000 replicates.
#
# SEED_OFFSET=200 shifts this batch's seeds to 201..400 so they do not
# collide with either earlier batch's seeds: the 2026-08-26 sweep used
# seeds 1..25, and the 2026-08-29 sweep used seeds 1..200.
#
# This is a fast/wide sweep budgeted for a plain 4-hour walltime (like
# the 2026-08-29 sweep), not the deep 36h/150,000-step budget used by
# the 2026-08-26 4-site sweep: N_STEPS=5000 is plenty at these rates
# for a shallow-but-broad replicate batch.
#
...
# folds 2000 replicates into ceil(2000 / 4) = 500 array tasks (under the
# 1000 cap) while keeping per-task walltime ~1x a single replicate.
#
# Global replicate index r in [0, N_TASKS): condition is
# MUTATION_RATES[r / 200], seed is (r % 200) + 1 + SEED_OFFSET. Array
# task t owns the CHUNK consecutive indices r = t * CHUNK + j for j in
# [0, CHUNK), each launched as a background job (indices >= N_TASKS are
# skipped on the final partial chunk). N_SITES is fixed at 4 (the
# 4-site model).
N_SITES=4
MUTATION_RATES=(
    1e-6 3e-7
...
echo "N_REPLICATES=${N_REPLICATES} N_CONDITIONS=${N_CONDITIONS} SEED_OFFSET=${SEED_OFFSET}"
...
# is recoverable downstream. SEED_OFFSET (${SEED_OFFSET}) shifts seeds
# to a range disjoint from both the 2026-08-26 (seeds 1..25) and
# 2026-08-29 (seeds 1..200) batches.
run_replicate() {
    local gid="\$1"
    local rate="\${MUTATION_RATES[\$((gid / ${N_REPLICATES}))]}"
    local seed="\$((gid % ${N_REPLICATES} + 1 + ${SEED_OFFSET}))"
