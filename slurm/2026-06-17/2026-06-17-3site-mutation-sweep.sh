...
# Sweep: the 3-site model across a geometric ladder of mutation rates
# (two points per decade, 1e-1 down to 1e-9 = 17 conditions) x 200
# replicates = 3400 array tasks.
# Task id i: MUTRATE_LIST[i / 200] for mutation_rate; (i % 200) + 1 for
# seed.
MUTRATE_LIST=(
    1e-1 3e-2 1e-2 3e-3 1e-3 3e-4 1e-4 3e-5 1e-5
    3e-6 1e-6 3e-7 1e-7 3e-8 1e-8 3e-9 1e-9
)
N_CONDITIONS=${#MUTRATE_LIST[@]}
N_REPLICATES=200
N_TASKS=$((N_CONDITIONS * N_REPLICATES))
N_SITES=3
N_STEPS=5000
POP_SIZE=100000
echo "MUTRATE_LIST=${MUTRATE_LIST[*]} N_REPLICATES=${N_REPLICATES}"
echo "N_CONDITIONS=${N_CONDITIONS} N_TASKS=${N_TASKS}"
echo "N_SITES=${N_SITES} N_STEPS=${N_STEPS} POP_SIZE=${POP_SIZE}"
...
MUTRATE_LIST=(${MUTRATE_LIST[*]})
TASK_ID=\${SLURM_ARRAY_TASK_ID:-0}
MUTRATE=\${MUTRATE_LIST[\$((TASK_ID / ${N_REPLICATES}))]}
SEED=\$((TASK_ID % ${N_REPLICATES} + 1))
echo "TASK_ID=\${TASK_ID} MUTRATE=\${MUTRATE} SEED=\${SEED}"

echo "do work ----------------------------------------------------- \${SECONDS}"
# Run the founder marimo notebook on CPU (engine=numpy, no GPU
# requested in #SBATCH) for the fixed 3-site model, sweeping the
# per-genome mutation rate via --mutation-rate. --skip-plotting=True
# drops the per-replicate teeplots --- the parquets are the deliverable.
# The notebook writes parquets to
# \${JOBDIR}/outdata/${NOTEBOOK_NAME}/<kind>/<uid>/... with simulation
# parameters (n_sites, pop_size, n_steps, seed, mutation_rate, ...) and a
# uuid replicate identifier stamped onto every row.
...
# spanning the full mutation-rate sweep. The strain parquet is the
# headline output --- one row per (replicate_uid, Step, strain) recording
# the per-genome infection count (n_cases) and new-infection count
# (n_new_infections) alongside the population-fraction counterparts. The
# hw parquet aggregates those per-genome counts by Hamming weight
# (n_cases per HW band).
